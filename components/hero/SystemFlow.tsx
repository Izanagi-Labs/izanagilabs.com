  ...
  return (
    <div className="relative flex flex-col items-center gap-6 lg:block w-full max-w-[340px] lg:max-w-none lg:w-[880px] lg:h-[500px] flex-shrink-0 mx-auto group">
      ...
      <motion.div 
        ...
        className="lg:hidden text-accent flex flex-col items-center my-4"
      >
         <ArrowDown size={20} strokeWidth={1.5} />
      </motion.div>

      {/* Mobile Workflow Wrapper */}
      <div className="flex flex-col gap-6 items-center lg:contents relative mt-4 lg:mt-0">
        <motion.div 
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: dDur(0.3), delay: d(1.1) }}
          className="lg:hidden flex flex-col items-center gap-1 mb-2"
        >
          <span className="text-system-xs text-foreground-fainter">Organized Workflows</span>
          <div className="w-4 h-[1px] bg-border" />
        </motion.div>
        
        <div className="flex items-center justify-center gap-6 lg:contents w-full">
          <WorkflowNode label="Capture" icon={<FileText size={15} />} x={420} y={200} className="relative place-self-auto lg:place-self-auto" initial={initialOrganized} animate={animateOrganized} transition={{ duration: dDur(0.4), delay: d(1.1) }} />
          
          {/* Right Arrow Mobile */}
          <motion.div 
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: dDur(0.3), delay: d(1.15) }}
            className="lg:hidden text-accent flex-shrink-0"
          >
            →
          </motion.div>

          <WorkflowNode label="Automate" icon={<Settings size={15} />} x={550} y={200} className="relative place-self-auto lg:place-self-auto" initial={initialOrganized} animate={animateOrganized} transition={{ duration: dDur(0.4), delay: d(1.2) }} />
        </div>
        ...
        <div className="flex items-center justify-center gap-6 lg:contents w-full">
          <WorkflowNode label="Review" icon={<CheckSquare size={15} />} x={680} y={200} className="relative place-self-auto lg:place-self-auto" initial={initialOrganized} animate={animateOrganized} transition={{ duration: dDur(0.4), delay: d(1.3) }} />
          ...
          <WorkflowNode label="Action" icon={<Play size={15} fill="currentColor" />} x={810} y={200} isActive className="relative place-self-auto lg:place-self-auto" initial={initialOrganized} animate={animateOrganized} transition={{ duration: dDur(0.4), delay: d(1.4) }} />
        </div>
      </div>
